...
bool TileMap::load(const std::string& tmx_file_path)
{
	...
	if (!texture->loadFromFile(path))
		return false;
	// создаем сетку, где индекс каждого тайла зависит от его координат в текстуре
		...
		for (size_t y = 0u, index = 0u; y < height; ++y)
		{
			for (size_t x = 0u; x < width; ++x, ++index)
			{
				...
				{   // обходим по часовой стрелке
					...
					// устанвливаем тайл в позицию на карте
	...
	{// получаем все данные - тип, имя, поизция, etc
